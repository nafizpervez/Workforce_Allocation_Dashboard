...
const STAGE_COLOR = {
  'Prospect': '#6B7280', 'Qualify': '#0EA5E9', 'Validate': '#8B5CF6',
  'Presentation - Solve': '#EC4899', 'Proposal': '#F59E0B',
  'Negotiate': '#F97316', 'Closed Won': '#10B981',
};
const STAGE_PILL = {
...
/** Format any USD amount as "16,000.00 USD" */
function fmtUsd(n) {
  const v = Number(n) || 0;
...
/** Expand [startDate, endDate] into a deduped list of weekly slots
 *  using W1=days 1-7, W2=8-14, W3=15-21, W4=22-end-of-month. */
function expandDateRange(start, end) {
  ...
  while (cur <= e) {
    ...
    const d = cur.getDate();
    const w = d <= 7 ? 1 : d <= 14 ? 2 : d <= 21 ? 3 : 4;
    const key = `${y}-${m}-${w}`;
    if (!seen.has(key)) { seen.add(key); out.push({ year: y, month: m, week: w }); }
    cur.setDate(d + 1);
  }
  return out;
}
...
async function loadAll() {
  try {
    ...
    /* Card drag must be (re-)initialised after dynamic stats render. */
    initCardDrag();
  } catch (e) {
...
function projectRowHtml(p) {
  const fte = (function () {
    const set = new Set();
    for (const a of S.assignments) if (a.project_id === p.id) set.add(a.employee_id);
    return set.size;
  })();
      ...
      <div class="ml-2">
        <div class="flex items-start justify-between gap-2 mb-1">
          <div class="min-w-0">
            ...
            <div class="text-xs text-gray-500">${esc(p.account_name || p.client || 'Internal')}</div>
          </div>
        </div>
        <div class="flex items-center gap-1.5 text-xs text-gray-600 mb-2 mono">
...
function renderPipeline(projects) {
  const active = projects.filter(p => p.stage !== 'Closed Won');
  document.getElementById('pipelineList').innerHTML =
    active.map(projectRowHtml).join('') || '<div class="px-6 py-8 text-center text-sm text-gray-400">No projects</div>';
}
...
function deadlineRowHtml(d) {
  ...
  const priC = PRIORITY_PILL[d.priority] || 'bg-gray-100 text-gray-700';
...
function renderDeadlines(data) {
  document.getElementById('deadlinesList').innerHTML =
    data.map(deadlineRowHtml).join('') || '<div class="px-6 py-8 text-center text-sm text-gray-400">No upcoming deadlines</div>';
}
...
function openModal(html, width = 'max-w-lg') {
  ...
  root.innerHTML = `<div class="fixed inset-0 modal-bd flex items-center justify-center z-50 p-4" id="mbd">
    <div class="bg-white rounded-2xl shadow-2xl w-full ${width} modal-enter" onclick="event.stopPropagation()">${html}</div>
  </div>`;
...
/* ── View All modal — shows full list of pipeline or deadlines ─── */
function openViewAllModal(type) {
  const isPipeline = type === 'pipeline';
  const title = isPipeline ? 'All Active Projects' : 'All Upcoming Deadlines';

  let items, listHtml;
  if (isPipeline) {
    items = S.projects.filter(p => p.stage !== 'Closed Won');
    listHtml = items.map(projectRowHtml).join('');
  } else {
    ...
    items = S.projects
      ...
      .map(p => {
        ...
        const days = Math.max(0, Math.round((due - today) / 864e5));
        const status = p.progress >= 80 ? 'On Track'
          : days < 14 && p.progress < 50 ? 'Delayed'
            : 'At Risk';
        return { ...p, days, status };
      });
    listHtml = items.map(deadlineRowHtml).join('');
  }

  openModal(`
    ${mHdr(title, `${items.length} item${items.length === 1 ? '' : 's'}`)}
    <div class="p-4">
      <div class="overflow-y-auto nice-scroll" style="max-height: 70vh">
        ${listHtml || '<div class="text-center text-gray-400 py-12">No items</div>'}
      </div>
    </div>
    <div class="p-4 border-t border-gray-200 flex justify-end bg-gray-50 rounded-b-2xl">
      <button onclick="closeModal()" class="btn-gray">Close</button>
    </div>
  `, 'max-w-3xl');
}

/* ── Assignment modal (Start Date + End Date + presets in CREATE) ─ */
function openAssignmentModal(opts = {}) {
  ...
  /* Default dates: pre-fill from the clicked cell if create-mode opens from matrix click. */
  const today = new Date();
  let defStart = today.toISOString().slice(0, 10);
  let defEnd = today.toISOString().slice(0, 10);
  if (!editing && opts.year && opts.month && opts.week) {
    const dStart = ({ 1: 1, 2: 8, 3: 15, 4: 22 })[opts.week];
    const dEnd = ({ 1: 7, 2: 14, 3: 21, 4: 28 })[opts.week];
    const sD = new Date(opts.year, opts.month - 1, dStart);
    const eD = new Date(opts.year, opts.month - 1, dEnd);
    defStart = sD.toISOString().slice(0, 10);
    defEnd = eD.toISOString().slice(0, 10);
  }

  /* EDIT mode: simple single-week edit */
  if (editing) {
    const monthLabel = `${MN[cur.month - 1]} ${cur.year} · Week ${cur.week}`;
    openModal(`
      ${mHdr('Edit Assignment', 'Update workload allocation for this week')}
      <div class="p-6 space-y-4">
        <div><label class="field-label">Employee</label>
          <select id="fa_emp" class="field-input">
            ${S.employees.map(e => `<option value="${e.id}" ${e.id === empId ? 'selected' : ''}>${esc(e.name)} – ${esc(e.dept)}</option>`).join('')}
          </select>
        </div>
        ...
        <div><label class="field-label">Period</label>
          <div class="field-input bg-gray-50 text-gray-700">${esc(monthLabel)}</div>
        </div>
        <div>
  ...
  /* CREATE mode: date range + presets + live slot count */
  openModal(`
    ${mHdr('Add Assignment', 'Assign an employee to a project across a date range')}
    <div class="p-6 space-y-4">
      <div><label class="field-label">Employee</label>
        <select id="fa_emp" class="field-input">
          ${S.employees.map(e => `<option value="${e.id}" ${e.id === empId ? 'selected' : ''}>${esc(e.name)} – ${esc(e.dept)}</option>`).join('')}
      ...
      <div class="grid grid-cols-2 gap-4">
        <div><label class="field-label">Start Date</label>
          <input id="fa_start" type="date" class="field-input" value="${defStart}" oninput="updateSlotPreview()">
        </div>
        <div><label class="field-label">End Date</label>
          <input id="fa_end" type="date" class="field-input" value="${defEnd}" oninput="updateSlotPreview()">
        </div>
      </div>
      ...
      <div>
        <label class="field-label flex justify-between"><span>Workload Allocation per Week</span><span class="text-blue-600 font-semibold" id="pctLbl">${pct}%</span></label>
        <input id="fa_pct" type="range" min="0" max="150" value="${pct}" class="w-full" oninput="document.getElementById('pctLbl').textContent=this.value+'%'">
        <div class="flex justify-between text-xs text-gray-400 mt-1"><span>0%</span><span>50%</span><span>100%</span><span>150%</span></div>
      </div>
    </div>
...
function setDateRange(preset) {
  ...
  if (preset === 'week') {
    s = new Date(today);
    e = new Date(today); e.setDate(e.getDate() + 6);
  } else if (preset === 'month') {
    s = new Date(today);
    e = new Date(today); e.setMonth(e.getMonth() + 1);
  } else if (preset === '3months') {
    s = new Date(today);
    e = new Date(today); e.setMonth(e.getMonth() + 3);
  } else if (preset === 'fiscalyear') {
    s = new Date(S.fiscalYear, 3, 1);           // April 1 of fiscal year
    e = new Date(S.fiscalYear + 1, 2, 31);      // March 31 next calendar year
  } else return;
  document.getElementById('fa_start').value = fmt(s);
  document.getElementById('fa_end').value = fmt(e);
  updateSlotPreview();
}

function updateSlotPreview() {
  const s = document.getElementById('fa_start');
  const e = document.getElementById('fa_end');
  const pv = document.getElementById('slotPreview');
  if (!s || !e || !pv) return;
  const slots = expandDateRange(s.value, e.value);
  pv.innerHTML = `Will create <span class="font-semibold">${slots.length}</span> weekly assignment${slots.length === 1 ? '' : 's'}`;
  pv.className = slots.length > 0
...
async function saveAssignment(id) {
  if (id) {
    /* Edit a single existing assignment */
    const payload = {
  ...
  /* Create — bulk over date range */
  const empId = +document.getElementById('fa_emp').value;
  const projId = +document.getElementById('fa_proj').value;
  const start = document.getElementById('fa_start').value;
  const end = document.getElementById('fa_end').value;
  const pct = +document.getElementById('fa_pct').value;

  if (!start || !end) { toast('Both start and end dates are required', 'error'); return; }
  const slots = expandDateRange(start, end);
  if (slots.length === 0) { toast('Invalid date range', 'error'); return; }

  try {
    const result = await api('POST', '/api/assignments/bulk', {
      employee_id: empId, project_id: projId, percentage: pct, slots
    });
    closeModal();
    toast(`Created ${result.created} weekly assignment${result.created === 1 ? '' : 's'}`);
    await loadAll();
  } catch (e) { toast(e.message, 'error'); }
}
...
function openEmployeeModal(opts = {}) {
  ...
  openModal(`
    ...
    <div class="p-6 space-y-4">
      ...
      <div><label class="field-label">Employee Email</label><input id="fe_email" type="email" class="field-input" value="${esc(email)}" placeholder="name@esribangladesh.com.bd"></div>
...
/* ── Project modal (SLIMMED — 13 fields total) ───────────────── */
function openProjectModal(opts = {}) {
  ...
  const OWNER_OPTS = [
    'Abdullah Al Baki',
    'Basher Muhammad Raquibul Raquibul',
    'Zobayer Ahmed',
    'Most Iffat Ara Ila',
    'Md Naiemul Haque Chowdhury',
    'Mohammad A. Hadi',
  ];
    ...
    <div class="p-6 space-y-4 max-h-[80vh] overflow-y-auto nice-scroll">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input id="fp_created" type="date" class="field-input" value="${esc(v('created_date', new Date().toISOString().slice(0, 10)))}">
        </div>
        <div>
...
async function saveProject(id) {
  ...
  const p = {
    ...
    client: document.getElementById('fp_account').value.trim(),
    ...
    created_date: document.getElementById('fp_created').value,
    progress: +document.getElementById('fp_prog').value,
    color: document.getElementById('fp_color').value,
  };
  if (!p.code || !p.name) { toast('Opportunity Number and Project Name are required', 'error'); return; }
  try {
...
function initEvents() {
  ...
  document.getElementById('matrixTable').addEventListener('click', e => {
    ...
    if (cell) openAssignmentModal({
      employee_id: +cell.dataset.emp,
      year: +cell.dataset.year, month: +cell.dataset.month, week: +cell.dataset.week,
    });
  });
  ...
  document.body.addEventListener('click', e => {
    ...
    if (va) openViewAllModal(va.dataset.viewAll);
  });
...
/* ── Section (vertical row) drag & drop ──────────────────────── */
function initSectionDrag() {
  const canvas = document.getElementById('dashboard-canvas');
  let dragSrc = null, lastTarget = null, lastIndicator = null;
  ...
  canvas.querySelectorAll('.ds').forEach(section => {
    const handle = section.querySelector(':scope > .drag-handle');
    if (!handle) return;
    ...
    section.addEventListener('dragstart', e => {
      /* Only trigger section drag if the drag started on the section's own handle.
         (Card drags will start from .dc-handle and stop propagation.) */
      if (!handle.matches(':active') && !section.hasAttribute('draggable')) return;
      dragSrc = section;
      e.dataTransfer.effectAllowed = 'move';
      e.dataTransfer.setData('text/plain', section.dataset.section || 'section');
    ...
    section.addEventListener('dragover', e => {
      ...
      if (dragSrc === section) { clearIndicators(); return; }
...
function initCardDrag() {
  document.querySelectorAll('.dc:not([data-drag-init])').forEach(card => {
    ...
    card.addEventListener('dragstart', e => {
      e.stopPropagation();  /* don't trigger the section drag */
      cardDragSrc = card;
      e.dataTransfer.effectAllowed = 'move';
    ...
    card.addEventListener('dragend', e => {
      ...
      document.querySelectorAll('.dc.drop-target').forEach(el => el.classList.remove('drop-target'));
      cardDragSrc = null;
    });

    card.addEventListener('dragover', e => {
      if (!cardDragSrc) return;
      if (cardDragSrc.parentElement !== card.parentElement) return;  /* only same row */
      e.preventDefault();
      e.stopPropagation();
      e.dataTransfer.dropEffect = 'move';
      document.querySelectorAll('.dc.drop-target').forEach(el => { if (el !== card) el.classList.remove('drop-target'); });
      if (card !== cardDragSrc) card.classList.add('drop-target');
    });
    ...
    card.addEventListener('drop', e => {
      ...
      if (cardDragSrc.parentElement !== card.parentElement) return;

      const parent = card.parentElement;
      const children = Array.from(parent.children);
      const srcIdx = children.indexOf(cardDragSrc);
      const dstIdx = children.indexOf(card);
      if (srcIdx < dstIdx) parent.insertBefore(cardDragSrc, card.nextSibling);
      else parent.insertBefore(cardDragSrc, card);
...
async function init() {
  initEvents();
  await loadAll();      /* loadAll() also calls initCardDrag() after stats render */
}
