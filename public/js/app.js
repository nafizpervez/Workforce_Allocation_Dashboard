  ...
  if (editing) {
    openModal(`${mHdr('Edit Assignment', 'Update workload allocation')}
      <div class="p-6 space-y-4">
        ...
        ${(() => { const proj = S.projects.find(p => p.id === cur?.project_id); const acct = proj ? (proj.account_name || proj.client || '') : ''; return acct ? `<div class="text-xs text-gray-500 -mt-2 px-1">Account: <span class="font-semibold text-gray-700">${esc(acct)}</span></div>` : ''; })()}
